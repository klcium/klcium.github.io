...
description: >-
  This blogpost will explain what I've learned on ELF dynamic entries and how it may be obfuscated on Android.
author: klcium
...
Not only this article surprised me as they already did all the research work more than a year earlier, but they also managed to hook the packing lib and dump the DEX files using the exact same techniques I had tried earlier.
...
Dumping the DEX files dynamically seemed pretty straightforward: Hook the decrypting function `decrypt_jar_128K` from the `libDexHelper.so` native lib and dump the memory. A less trivial approach would consist in reversing the encryption mechanisms and rebuild the DEX files locally.

> Note: in DJI Fly, past 1.12 ~ 1.13 the libDexHelper.so app is renamed libAppGuard.so. But it is the same thing actually.


Since both apps are packed by SecNeo, all I have to do was to reproduce theses steps. **Easy ..right?**
...
This technique did not work, and the app no longer booted. To make sure this was not due to the injected gadget, I simply read and saved the program using LIEF without editing the PT_DYNAMIC table (I will explain this concept further down this article) and yielded the same results.
...
I eventually gave up and might come back to it later if I get more skilled at RE. To be fair, progress was slow, and spending more time trying to understand this wasn’t a priority.
...
Unfortunately, their article does not provide more information on how they achieved it. But at least, I now have a proof of the feasibility of this technique and that all I have to do now is *try harder™*.
...
``` 
Hm, that's interesting. I thought sections should not be trusted. Why doesn't the program search for the DT_DYNAMIC instead? The answer is obviously more nuanced than this.  
...
```
The comment is clear: the `p_vaddr` attribute of the dynamic segment is used to find the dynamic table.
LIEF even goes further than this as they also compute the relative offset of the dynamic table based on the `PT_LOAD` segment that contains the dynamic table ([source](https://github.com/lief-project/LIEF/blob/abcf929efb748c7846dd59007cbb807e108db311/src/ELF/Parser.tcc#L233)).

> Note: If no wrapping `PT_LOAD` segment is found, the parser falls back to reading directly from the file offset of the `PT_DYNAMIC` segment.

#### ELF File building process

Understanding how LIEF rebuilds ELF files was a daunting task. Instead, we’ll learn in the next chapter how it works by comparing the results of rebuilding multiple files.
...
Something is off. 
Let's do a double hexdump at the offset of the two tables. The left part of the array is the `d_tag` which corresponds to the type of the entry. The right part of the array is the index in the string table and references the name of the entry.
...
```
Wait, we have two dynamic tables! The first 7 entries are identical, but the second one has some more entries following.
...
There is only one value we can trust : `p_vaddr = 0x0E8720`
And this is where our *actual* dynamic table is*.

> *Once the possible offset of the `PT_LOAD` segment that contains the dynamic table has been taken into account.
...
Before injecting with LIEF which may be blocked by some file integrity checks, I tried to change the offset of a `DT_NEEDED` entry for a random string included in the binary. So i naturally edited the `0xE8720` table (the `p_vaddr` one) and ended up with NO error upon loading.  
After this and from a clean binary, I did the same operation on the `p_offset` entry (`0xEFB90`) and did get an error message saying the library does not exists:
...
Yes 🥺👉👈. To understand why, let's look at the source code of the Android linker.
...
**Why can't `readelf` find the `.dynamic` section?**  
Because it searches for the section name `.dynamic`. In our case, all section names are stripped.
...
`DT_NEEDED` entries are parsed from the sections! In Android, the `.dynamic` section entry must be at the same `p_offset` and size of the `PT_DYNAMIC` entry. A `DT_STRTAB` is not required as it is found via the `sh_link` attribute of the `SHT_DYNAMIC`.
