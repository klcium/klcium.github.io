...
⚠️ Segments are required to run the binary
...
⚠️ Sections are not required for runtime
...
Yipee dance ! But we're not done yet.
...
After this and from a clean binary, I did the same operation on the `p_offset` entry (`0xEFB90`) and did get an error message saying the lib does not exists:
...
So you are saying both dynamic tables actually used to link our binary ???
...
The [find_libraries()](https://cs.android.com/android/platform/superproject/+/android-latest-release:bionic/linker/linker.cpp;drc=7ea4c99263fb09c4590135e78bc26fa4e5786083;l=1545) function from the bionic linker works the following way:

**1. LOADING PHASE**  
ELF files are mapped into memory and dependencies discovered. 
...
- calls [load_library()](https://cs.android.com/android/platform/superproject/+/android-latest-release:bionic/linker/linker.cpp;drc=7ea4c99263fb09c4590135e78bc26fa4e5786083;l=1311)
  ...
  - Uses `ElfReader`:
    ...
    > ⚠️ It parses the `SHT_DYNAMIC` value that **must** be equal to the **p_offset** and have the **same size** as in the program header.  
    ...
    > Later the linker will call the getter [ElfRader::Dynamic()](https://cs.android.com/android/platform/superproject/+/android-latest-release:bionic/linker/linker.cpp;drc=7ea4c99263fb09c4590135e78bc26fa4e5786083;l=1299) to get this data.
...
- [prelink_image()](https://cs.android.com/android/platform/superproject/+/android-latest-release:bionic/linker/linker.cpp;drc=7ea4c99263fb09c4590135e78bc26fa4e5786083;l=2852):
  ...
  - Also sets up `soinfo->strtab`, `soinfo->symtab`, `soinfo->plt_got`, etc. basde on the `PT_DYNAMIC` dynamic table.
  > ⚠️ A `DT_HASH`/`DT_GNU_HASH`, `DT_SYMTAB` and `DT_STRTAB` are actuallly required in the `PT_DYNAMIC` DT for the lib to be loaded.
...
**In `readelf` after injection, where all these new dynamic entries come from ?**  
LIEF parsed the corred dynamic entry. Upon modification it wrote the `p_vaddr` dynamic at the position of the `p_offset` of the `PT_DYNAMIC` entry.
The two dynamic tables have a different size. The `p_vaddr` one was larger and overflowed over the section header table.
...
The linker requires the `p_offset` of the `PT_DYNAMIC` to match the `p_addr` of the `SHT_DYNAMIC`. Hence why there are two different values in the `PT_DYNAMIC` ([source](https://android.googlesource.com/platform/bionic/+/master/android-changes-for-ndk-developers.md#missing-section-headers-enforced-for-api-level-24)).
